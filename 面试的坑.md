...
子节点共用的事件可以委托给其父节点，这样只要绑定一次事件而不用每个子元素都绑定，同时还可以实现事件的动态绑定
...
[es6 模块与commonJS的区别](https://www.cnblogs.com/murenziwei/p/13266812.html)

#### (1). 加载方式

es6为编译时加载，commonjs是运行时加载，这就导致了：

* 使用时es6模块可以通过export命令显式指定要输出的代码，而commonjs只能导入一整个对象
* 在加载时es6可以只加载暴露出的方法，其他方法可以不加载，而commonjs会加载整个文件，es6效率比commonjs高
...
https://juejin.cn/post/6942060751272017928
